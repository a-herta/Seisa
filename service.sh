...
cleanup() {
  ...
  if [ -x "$TPROXY" ]; then
    sh "$TPROXY" stop >/dev/null 2>&1 || log_safe "❓ 防火墙规则清理失败"
  fi
  ...
  if command -v pkill >/dev/null 2>&1; then
    log_safe "🛑 终止守护进程..."
    pkill -f "$(basename "$MONITOR")" 2>/dev/null || true
  fi
  log_safe "✨ 残留服务清理完成"
}
...
apply_rules() {
  if [ -x "$TPROXY" ]; then
    log_safe "🔥 正在应用网络规则..."
    sh "$TPROXY" >/dev/null 2>&1 || {
      log_safe "❓ 网络规则应用失败"
      return 1
    }
  else
    log_safe "❌ 网络规则未找到, 请重新安装模块"
    return 1
  fi
}
